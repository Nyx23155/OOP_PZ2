...
int main() {
    ...
    addPerson(allPeople, "Ivan Petrenko", "M");     // Індекс 0
    addPerson(allPeople, "Maria Koval", "W");       // Індекс 1
    addPerson(allPeople, "Oleksiy Sydorenko", "M"); // Індекс 2
    addPerson(allPeople, "Olena Bondarenko", "W");  // Індекс 3
    addPerson(allPeople, "Sergiy Melnyk", "M");     // Індекс 4
    addPerson(allPeople, "Natalia Shevchenko", "W");// Індекс 5

    vector<Place*> allPlaces;
    addPlace(allPlaces, "Home", "Kyiv", "Ukraine");           // Індекс 0
    addPlace(allPlaces, "Arcadia Beach", "Odessa", "Ukraine");  // Індекс 1
    addPlace(allPlaces, "Carpathians", "Lviv", "Ukraine");      // Індекс 2
    addPlace(allPlaces, "Shevchenko Park", "Dnipro", "Ukraine"); // Індекс 3
    addPlace(allPlaces, "Freedom Square", "Kharkiv", "Ukraine");// Індекс 4
    addPlace(allPlaces, "Gorky Park", "Kharkiv", "Ukraine");   // Індекс 5
    addPlace(allPlaces, "Eiffel Tower", "Paris", "France");     // Індекс 6

    vector<Album*> allAlbums;
    addAlbum(allAlbums, "Vacation");    // Індекс 0
    addAlbum(allAlbums, "Family");      // Індекс 1
    addAlbum(allAlbums, "Landscapes");  // Індекс 2

    vector<Photo*> allPhotos;
    auto now = chrono::system_clock::now();

    // Фото для статистики по ЗИМІ
    ...
    // Ще одне літнє фото, щоб зробити Івана і Марію "друзями"
    ...
    // Ще одне фото, щоб зробити Івана і Марію "друзями"
    ...
    // Фото річної давності для перевірки фільтра по даті
    ...
    // Групове фото, щоб підняти популярність Марії та міста Харків
    ...
    // Пейзажні фото для альбому "Landscapes" (без людей)
    ...
    // Перевірка логіки додавання фото, яке вже є в іншому альбомі
    cout << "\n--- Testing album logic ---" << endl;
    allAlbums[1]->addPhoto(allPhotos[0]); // Намагаємось додати winter_lviv.jpg (вже у Vacation) в Family
    cout << "-------------------------\n" << endl;
